...
const defaultState: ProjectState = {
  ...
  batterieChoisie: null,
  onduleurParams: { margePourcent: 20 },
  onduleurChoisi: null,
  regulateurParams: { type: 'MPPT', margePourcent: 25, calibreChoisi: 60 },
  regulateurChoisi: null,
  cablage: { longueurAllerM: 10, chuteTensionMaxPourcent: 3 },
  cableChoisi: null,
  protectionChoisie: null,
};
...
interface ProjectContextValue {
  ...
  updateCablage: (patch: Partial<ProjectState['cablage']>) => void;
  setPanneauChoisi: (v: ProjectState['panneauChoisi']) => void;
  setBatterieChoisie: (v: ProjectState['batterieChoisie']) => void;
  setOnduleurChoisi: (v: ProjectState['onduleurChoisi']) => void;
  setRegulateurChoisi: (v: ProjectState['regulateurChoisi']) => void;
  setCableChoisi: (v: ProjectState['cableChoisi']) => void;
  setProtectionChoisie: (v: ProjectState['protectionChoisie']) => void;
  addAppareil: () => void;
...
export function ProjectProvider({ children }: { children: ReactNode }) {
  ...
  const updateCablage: ProjectContextValue['updateCablage'] = (patch) =>
    setState((s) => ({ ...s, cablage: { ...s.cablage, ...patch } }));

  const setPanneauChoisi: ProjectContextValue['setPanneauChoisi'] = (v) => setState((s) => ({ ...s, panneauChoisi: v }));
  const setBatterieChoisie: ProjectContextValue['setBatterieChoisie'] = (v) => setState((s) => ({ ...s, batterieChoisie: v }));
  const setOnduleurChoisi: ProjectContextValue['setOnduleurChoisi'] = (v) => setState((s) => ({ ...s, onduleurChoisi: v }));
  const setRegulateurChoisi: ProjectContextValue['setRegulateurChoisi'] = (v) => setState((s) => ({ ...s, regulateurChoisi: v }));
  const setCableChoisi: ProjectContextValue['setCableChoisi'] = (v) => setState((s) => ({ ...s, cableChoisi: v }));
  const setProtectionChoisie: ProjectContextValue['setProtectionChoisie'] = (v) => setState((s) => ({ ...s, protectionChoisie: v }));
