    ...
    <div>
      ...
      <div className="overflow-x-auto -mx-4 sm:mx-0">
        <table className="w-full text-sm min-w-[700px]">
          <thead>
            <tr className="text-left text-xs text-ink/60 border-b border-forest-200">
              <th className="py-2 pr-2 font-medium">Appareil</th>
              <th className="py-2 px-2 font-medium">Puissance (W)</th>
              <th className="py-2 px-2 font-medium">Qté</th>
              <th className="py-2 px-2 font-medium">h/jour</th>
              <th className="py-2 px-2 font-medium">Coeff.</th>
              <th className="py-2 px-2 font-medium">Démarrage important</th>
              <th className="py-2 px-2 font-medium text-right">Énergie/jour</th>
              <th className="py-2 pl-2" />
            </tr>
          </thead>
          <tbody>
            {state.appareils.map((a) => (
              <tr key={a.id} className="border-b border-forest-100">
                <td className="py-2 pr-2">
                  <input
                    className="w-full min-w-[120px] rounded border border-forest-200 px-2 py-1.5"
                    value={a.nom}
                ...
                <td className="py-2 px-2">
                  <input
                    type="number"
                    className="w-24 rounded border border-forest-200 px-2 py-1.5 font-mono-num"
                    value={a.puissanceW}
                    min={0}
                ...
                <td className="py-2 px-2">
                  <input
                    type="number"
                    className="w-16 rounded border border-forest-200 px-2 py-1.5 font-mono-num"
                    value={a.quantite}
                    min={1}
                    onChange={(e) => updateAppareil(a.id, { quantite: Math.max(1, parseInt(e.target.value) || 1) })}
                  />
                </td>
                <td className="py-2 px-2">
                  <input
      ...
      <p className="text-xs text-ink/55 mt-3 max-w-lg leading-relaxed">
        Le coefficient représente le fonctionnement moyen réel de l'appareil (par défaut 1 = fonctionnement continu
        pendant les heures indiquées). Il n'est jamais appliqué automatiquement sans être visible ici.
      </p>

      <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Stat label="Puissance totale" value={`${bilan.puissanceTotaleW.toFixed(0)} W`} />
        ...
        <Stat label="Énergie / an" value={`${(bilan.energieAnneeWh / 1000).toFixed(0)} kWh`} />
      </div>
