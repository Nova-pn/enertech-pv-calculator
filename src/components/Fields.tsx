...
export function Spec({ label, value, mono = true }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      <dt className="text-ink/50">{label}</dt>
      <dd className={mono ? 'font-mono-num text-forest-950' : 'text-forest-950'}>{value}</dd>
    </div>
  );
}
