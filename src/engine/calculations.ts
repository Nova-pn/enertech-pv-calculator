...
export function energieQuotidienneAppareil(a: Appareil): number {
  if (
    !Number.isFinite(a.puissanceW) ||
    a.puissanceW <= 0 ||
    !Number.isFinite(a.quantite) ||
    a.quantite < 1 ||
    !Number.isFinite(a.heuresParJour) ||
    a.heuresParJour < 0 ||
    a.heuresParJour > 24 ||
    !Number.isFinite(a.coefficient)
  ) {
    return 0;
  }
...
export function calculerBilan(appareils: Appareil[]): BilanConsommation {
  const puissanceValide = (a: Appareil) => (Number.isFinite(a.puissanceW) ? a.puissanceW : 0) * (Number.isFinite(a.quantite) ? a.quantite : 0);
  const puissanceTotaleW = appareils.reduce((s, a) => s + puissanceValide(a), 0);
...
export function puissanceSimultanee(appareils: Appareil[]): number {
  return appareils.reduce((s, a) => s + (Number.isFinite(a.puissanceW) ? a.puissanceW : 0) * (Number.isFinite(a.quantite) ? a.quantite : 0), 0);
}
...
export function calculerRegulateurPWM(
  ...
  margePourcent: number,
  calibreChoisiA: number
): DimensionnementRegulateurPWM {
  const courantAvecMargeA = courantIscChampA * (1 + margePourcent / 100);
  return {
    courantIscChampA,
    courantAvecMargeA,
    calibreSuffisant: calibreChoisiA >= courantAvecMargeA,
...
/**
 * Chute de tension en ligne DC (aller-retour) : ΔV = 2 × L(m) × I(A) × R(Ω/km) / 1000.
 * Le pourcentage est rapporté à la tension de référence du champ (Vmp) qui est la tension
 * de fonctionnement réelle du câblage PV.
 */
export function calculerChuteDeTension(
  longueurAllerM: number,
  courantA: number,
  resistanceOhmPerKm: number,
  tensionReferenceV: number,
  seuilMaxPourcent: number
): ChuteDeTension {
  if (longueurAllerM <= 0 || courantA <= 0 || resistanceOhmPerKm <= 0 || tensionReferenceV <= 0) {
    return { chuteVoltsV: 0, chutePourcent: 0, acceptable: true };
  }
  const chuteVoltsV = 2 * longueurAllerM * courantA * (resistanceOhmPerKm / 1000);
  const chutePourcent = (chuteVoltsV / tensionReferenceV) * 100;
  return { chuteVoltsV, chutePourcent, acceptable: chutePourcent <= seuilMaxPourcent };
}

// ---------------------------------------------------------------------------
// 9. Protection DC — calibre recommandé
// ---------------------------------------------------------------------------

export interface CalibreProtection {
  calibreRecommandeA: number;
}

/** Calibre recommandé pour la protection côté champ PV : 1,25 × Isc du champ (pratique courante IEC/NEC). */
export function calculerCalibreProtection(courantIscChampA: number): CalibreProtection {
  return { calibreRecommandeA: courantIscChampA * 1.25 };
}

// ---------------------------------------------------------------------------
// 10. Section de câble théorique et sélection normalisée
// ---------------------------------------------------------------------------

/** Résistivité du cuivre, constante physique standard (Ω·mm²/m) — non spécifique à un fabricant. */
export const RESISTIVITE_CUIVRE_OHM_MM2_PAR_M = 0.0175;

/**
 * S = (2 × L × I × ρ) / ΔV — section théorique en mm² du conducteur (aller-retour).
 * Renvoie null si une donnée nécessaire est absente ou invalide : on ne doit jamais laisser
 * croire qu'un calcul a été fait avec une valeur par défaut inventée.
 */
export function calculerSectionTheorique(
  longueurAllerM: number,
  courantA: number,
  chuteVoltsAdmissible: number,
  resistiviteOhmMm2ParM: number = RESISTIVITE_CUIVRE_OHM_MM2_PAR_M
): number | null {
  if (
    !Number.isFinite(longueurAllerM) ||
    longueurAllerM <= 0 ||
    !Number.isFinite(courantA) ||
    courantA <= 0 ||
    !Number.isFinite(chuteVoltsAdmissible) ||
    chuteVoltsAdmissible <= 0
  ) {
    return null;
  }
  return (2 * longueurAllerM * courantA * resistiviteOhmMm2ParM) / chuteVoltsAdmissible;
}

/**
 * Sélectionne, parmi une liste de sections déjà prévues dans l'application, la plus petite
 * supérieure ou égale à la section théorique. Ne choisit jamais une section inférieure et
 * n'invente aucune valeur : renvoie null si aucune section du catalogue ne convient.
 */
export function selectionnerSectionNormalisee(sectionTheoriqueMm2: number, sectionsDisponiblesMm2: number[]): number | null {
  const candidates = sectionsDisponiblesMm2.filter((s) => s >= sectionTheoriqueMm2).sort((a, b) => a - b);
  return candidates.length > 0 ? candidates[0] : null;
}

/**
 * Sélectionne, parmi une liste de calibres déjà prévus, le plus petit supérieur ou égal
 * au calibre recommandé. Même principe que pour les sections de câble.
 */
export function selectionnerCalibreNormalise(calibreRecommandeA: number, calibresDisponiblesA: number[]): number | null {
  const candidates = calibresDisponiblesA.filter((c) => c >= calibreRecommandeA).sort((a, b) => a - b);
  return candidates.length > 0 ? candidates[0] : null;
}
...
export function construireAvertissements(args: {
  ...
  regulateur: DimensionnementRegulateur | DimensionnementRegulateurPWM;
  regulateurType?: 'MPPT' | 'PWM';
  chuteDeTension?: ChuteDeTension | null;
  calibreProtection?: { recommandeA: number; choisi: { min: number | null; max: number | null; tensionV: number }; vocChampV: number } | null;
}): Avertissement[] {
  const { configPV, configBatterie, onduleur, regulateur, regulateurType, chuteDeTension, calibreProtection } = args;
  ...
  if (chuteDeTension) {
    if (chuteDeTension.acceptable) {
      ...
      avertissements.push({
        niveau: 'orange',
        titre: 'Chute de tension du câblage élevée',
        explication: `${chuteDeTension.chutePourcent.toFixed(2)} % estimés, au-dessus du seuil visé. Augmentez la section du câble ou réduisez la longueur de ligne.`,
      });
    }
  }

  if (calibreProtection) {
    const { recommandeA, choisi, vocChampV } = calibreProtection;
    const courantOk =
      (choisi.min === null || choisi.min <= recommandeA) && (choisi.max === null || choisi.max >= recommandeA);
    const tensionOk = choisi.tensionV >= vocChampV;
    if (courantOk && tensionOk) {
      avertissements.push({
        niveau: 'vert',
        titre: 'Protection DC compatible',
        explication: `Calibre recommandé ${recommandeA.toFixed(1)} A (1,25 × Isc du champ), couvert par la protection choisie (${choisi.tensionV} V DC).`,
      });
    } else {
      const raisons: string[] = [];
      if (!courantOk) raisons.push(`le calibre recommandé de ${recommandeA.toFixed(1)} A sort de la plage réglable de la protection choisie`);
      if (!tensionOk) raisons.push(`la tension Voc du champ (${vocChampV.toFixed(1)} V) dépasse la tension admissible de la protection (${choisi.tensionV} V DC)`);
      avertissements.push({
        niveau: 'rouge',
        titre: 'Protection DC à revoir',
        explication: `Vérifiez la protection choisie : ${raisons.join(' et ')}.`,
      });
    }
  }
